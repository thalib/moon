...
- Define service objects in the `src/ruby/services` directory to encapsulate business logic.
- Use form objects located in `src/ruby/forms` to manage validation and submission logic.
- Implement JSON serializers in the `src/ruby/serializers` directory to format API responses.
- Define authorization policies in `src/ruby/policies` to control user access to resources.
- Structure the GraphQL API by organizing schemas, queries, and mutations inside `src/ruby/graphql`.
- Create custom validators in `src/ruby/validators` to enforce specialized validation logic.
- Isolate and encapsulate complex ActiveRecord queries in `src/ruby/queries` for better reuse and testability.
- Define custom data types and coercion logic in the `src/ruby/types` directory to extend or override ActiveModel type behavior.
...
- Use `src/ruby/javascript` as the main directory for managing JavaScript packs, modules, and frontend logic if needed.
